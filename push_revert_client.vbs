...
' Stage package.json, commit, push
WshShell.Run "cmd /c cd C:\Users\user\Documents\your-trip && " & _
    ...
    "git commit -m ""fix: revert @prisma/client to ^7.3.0 to match lock file — matches working 10995ed"" && " & _
    "git push github main " & _
    "> C:\Users\user\Documents\your-trip\push_revert_output.txt 2>&1", 1, True

' Show result
Dim outLog
outLog = ""
If fso.FileExists("push_revert_output.txt") Then
    Dim f
    Set f = fso.OpenTextFile("push_revert_output.txt", 1)
    outLog = f.ReadAll
    f.Close
End If
MsgBox "Revert Push Result:" & Chr(10) & outLog, 64, "YourTrip"
